...
import { Plus, Target, Crown } from "lucide-react";
import { GlassCard, BottomSheet } from "./DayLensUI";
import type { Goal, DayEntry } from "@/lib/daylens-constants";
import { scoreGradient, getMetricVal } from "@/lib/daylens-utils";
...
export const GoalsScreen = ({ goals, setGoals, recent, isPremium, onShowPricing }: GoalsScreenProps) => {
        ...
        return (
          <GlassCard key={g.id}>
            <div className="flex justify-between items-start mb-4">
              <div><h3 className="text-base font-medium">{g.label}</h3><p className="text-xs text-muted-foreground mt-1">Target: {g.op === "gte" ? "≥" : "≤"} {g.target} {g.unit} · Last 7 days</p></div>
