...
export const PaywallScreen = ({ onSubscribe }: PaywallProps) => {
  ...
  return (
    <div className="max-w-md mx-auto min-h-screen relative bg-background flex flex-col overflow-hidden">
      {/* Ambient glow */}
      <div className="fixed top-20 -left-20 w-80 h-80 bg-primary/[0.06] blur-[120px] rounded-full pointer-events-none" />
      <div className="fixed bottom-32 -right-20 w-72 h-72 bg-primary/[0.04] blur-[100px] rounded-full pointer-events-none" />
      ...
      <div className="flex-1 flex flex-col pt-16 px-6">
        ...
        >
          <div className="w-20 h-20 rounded-3xl bg-gradient-to-br from-primary/80 to-primary flex items-center justify-center mb-5 shadow-lg shadow-primary/30">
            <Crown className="text-primary-foreground" size={36} />
          </div>
          <h1 className="font-display text-3xl font-extrabold tracking-tight mb-2">
            Unlock daygo<span className="text-primary">.ai</span>
          </h1>
            ...
            <button
              ...
              className={`w-full rounded-2xl p-4 text-left transition-all flex items-center justify-between ${
                selectedPlan === plan.id
                  ? "bg-primary/[0.12] border-2 border-primary/30"
                  : "bg-muted/10 border-2 border-transparent hover:border-muted/20"
              }`}
            >
              <div>
                <div className="flex items-center gap-2 mb-0.5">
                  <span className={`text-[15px] font-bold ${selectedPlan === plan.id ? "text-primary" : "text-foreground/80"}`}>
                    {plan.label}
                  </span>
                  {plan.savings && (
              ...
              <div className="flex items-center gap-1.5">
                <span className={`text-xl font-extrabold font-display ${selectedPlan === plan.id ? "text-primary" : "text-foreground/60"}`}>
                  {plan.price}
                </span>
      ...
      <div className="px-6 pb-10 pt-4 space-y-3 z-10">
        <button
          ...
          className="w-full h-14 rounded-2xl font-display font-extrabold text-[15px] flex items-center justify-center gap-2 bg-primary text-primary-foreground hover:opacity-90 active:scale-[0.98] transition-all"
        >
